...
<section>
  ...
  <form method="post" action="faculty.php">
    ...
    <input type="number" id="Faculty_ID" name="Faculty_ID" required><br><br>
    ...
    <label for="Lecturer_names">Lecturer's Names:</label>
    <input type="text" id="Lecturer_names" name="Lecturer_names" required><br><br>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO Faculty(Faculty_ID, Name, Contact_Information, course_id, Lecturer_names) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $Faculty_ID, $Name, $Contact_Information, $course_id, $Lecturer_names);
  ...
  <table border="5">
    ...
    // Prepare SQL query to retrieve all Faculty
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        $Faculty_ID = $row['Faculty_ID']; // Fetch the Faculty_ID
        echo "<tr>
                  <td>" . $row['Faculty_ID'] . "</td>
                  <td>" . $row['Name'] . "</td>
                  <td>" . $row['Contact_Information'] . "</td>
                  <td>" . $row['course_id'] . "</td>
                  <td>" . $row['Lecturer_names'] . "</td>
                  <td><a style='padding:4px' href='delete_Faculty.php?Faculty_ID=$Faculty_ID'>Delete</a></td> 
                  <td><a style='padding:4px' href='update_Faculty.php?Faculty_ID=$Faculty_ID'>Update</a></td> 
                </tr>";
      }
    } else {
